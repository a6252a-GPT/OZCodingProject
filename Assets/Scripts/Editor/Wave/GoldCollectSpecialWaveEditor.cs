...
{
    [CustomEditor(typeof(GoldCollectSpecialWave))]
    public sealed class GoldCollectSpecialWaveEditor : Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            WaveInspectorUtility.DrawSection("특수웨이브 등장", "최소 Stage 이후 확률로 골드 수집 특수웨이브가 시작됩니다. 실패할 때마다 확률이 조금씩 올라갑니다.");
            WaveInspectorUtility.DrawProperty(serializedObject, "enableSpecialWave", "골드 특수웨이브 사용");
            ...
            WaveInspectorUtility.DrawPercentProperty(serializedObject.FindProperty("chanceIncreaseOnFailPercent"), "실패 시 증가 확률");
            ...
            WaveInspectorUtility.DrawSection("골드 수집", "Nexus 주변에 골드 오브젝트를 뿌리고, 제한 시간 동안 먹은 비율을 계산합니다.");
            WaveInspectorUtility.DrawProperty(serializedObject, "goldPickupPrefab", "골드 오브젝트 Prefab");
            WaveInspectorUtility.DrawProperty(serializedObject, "goldRoot", "골드 생성 부모");
            WaveInspectorUtility.DrawProperty(serializedObject, "nexus", "Nexus 기준 위치");
            WaveInspectorUtility.DrawProperty(serializedObject, "goldSpawnCount", "골드 생성 개수");
            ...
            WaveInspectorUtility.DrawProperty(serializedObject, "goldHeightOffset", "골드 높이 보정");
            WaveInspectorUtility.DrawProperty(serializedObject, "collectRadius", "획득 거리");
            WaveInspectorUtility.DrawProperty(serializedObject, "goldRewardPerPickup", "골드 1개당 지급 골드");
            ...
            WaveInspectorUtility.DrawSection("보상 상자", "BonusChest 프리팹을 연결합니다. 상자 열기/보상 처리는 기존 BonusChest 쪽 로직을 사용합니다.");
        ...
        private void RunForTargets(System.Action<GoldCollectSpecialWave> action)
        {
            foreach (Object targetObject in targets)
            {
                if (targetObject is GoldCollectSpecialWave wave)
                {
                    action?.Invoke(wave);
